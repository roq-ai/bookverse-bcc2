...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'book.create': {
    ...
    key: 'new-book-added',
    tenantPath: ['publisher', 'book'],
    userPath: [],
  },
  'review.create': {
    roles: ['reader', 'contributor'],
    key: 'new-review-posted',
    tenantPath: ['publisher', 'book', 'review'],
    userPath: [],
  },
  'discussion.create': {
    roles: ['reader', 'contributor'],
    key: 'new-discussion-started',
    tenantPath: ['publisher', 'book', 'discussion'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['reader'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['reader', 'contributor'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.publisher.findFirst({
