...
import { getDiscussions } from 'apiSdk/discussions';
import { DiscussionInterface } from 'interfaces/discussion';

import { CollectionListPage as ShowCaseList } from 'pages/collections';

function DashboardPage() {
  const { session } = useSession();

  const bookStat = useSWR<PaginatedInterface<BookInterface>>(
    () => `/books`,
    () => getBooks({ limit: 1, offset: 0 }),
  );
  ...
  const discussionStat = useSWR<PaginatedInterface<DiscussionInterface>>(
    () => `/discussions`,
    () => getDiscussions({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Books',
      result: bookStat?.data?.totalCount,
    ...
    {
      title: 'Discussions',
      result: discussionStat?.data?.totalCount,
      isLoading: discussionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
